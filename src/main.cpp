...
uint256 hashGenesisBlock("0xedc52fb4d1433f50f8664a56724b96e87e8ea63eeb8ca7111c86994cba110dc0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Autocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Autocoin Signed Message:\n";
...
{
    // Autocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if (nHeight == 1)
	{
		// For giveaway, marketing and introduce to auto garage
		nSubsidy = 10000 * COIN;
...
static const int64 nTargetSpacing = 40; // Navyriya: 2 minutes
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "AutoCoin 02/03/2014 In Mean, AutoCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1391361068;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085668738;
        ...
        assert(block.hashMerkleRoot == uint256("0x89d101c693e7d7f0f115d8814575c1697e06e250c409234ed7322327b9a5b52b"));
...
unsigned char pchMessageStart[4] = { 0xda, 0xa2, 0xc8, 0xaa }; // Litecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AutocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AutocoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("AutocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("autocoin-miner");
    ...
    try { while(true) {
        ...
        printf("Running AutocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AutocoinMiner terminated\n");
        throw;
    }
}
